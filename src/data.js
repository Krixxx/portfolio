export const timeLineElements = [
  {
    id: 1,
    title: 'MERN portfolio app',
    subtitle: 'React, Node, Express, MongoDB',
    description: 'Social media app for portfolio example',
    buttonText: 'Go to projects',
    date: 'November 2021',
    icon: 'app',
  },
  {
    id: 2,
    title: 'Learn Node, Express, MongoDB',
    subtitle: 'Udemy course',
    description: 'Started learning back-end development',
    date: 'October 2021',
    icon: 'study',
  },
  {
    id: 3,
    title: 'Production Notification app',
    subtitle: 'React, Firebase Database',
    description: 'Production notification and live feedback application',
    buttonText: 'Go to projects',
    date: 'October 2021',
    icon: 'app',
  },
  {
    id: 4,
    title: 'Learn React JS',
    subtitle: 'Udemy course',
    description: 'Started learning JavaScript framework',
    date: 'May 2021',
    icon: 'study',
  },
  {
    id: 5,
    title: 'Real-estate website',
    subtitle: 'HTML, CSS, JavaScript',
    description:
      'Fully functioning real-estate website. Custom floor plan explorer.',
    buttonText: 'Go to projects',
    date: 'September 2020',
    icon: 'app',
  },
  {
    id: 6,
    title: 'Learn HTML, CSS, JavaScript',
    subtitle: 'Udemy course',
    description: 'Started learning web application development',
    date: 'April 2020',
    icon: 'study',
  },
  {
    id: 2,
    title: 'Warehouse management app',
    subtitle: 'Java',
    description:
      'Android app for special warehouse management. For receiving products and for releasing goods for production',
    buttonText: 'Go to projects',
    date: 'January 2020',
    icon: 'app',
  },
  {
    id: 1,
    title: 'Started learning Android Development with Java',
    subtitle: 'Udemy course',
    description: 'My first "Hello world" app',
    date: 'July 2019',
    icon: 'study',
  },
];
